...
// Move initializeMetisService inside the component or create a separate hook
const initializeMetisService = async () => {
    try {
        const apiManager = APIIntegrationManager.getInstance();
        
        const metisService = {
            name: 'Metis AI Service',
            url: process.env.REACT_APP_METIS_API_URL,
            key: process.env.REACT_APP_METIS_API_KEY,
            version: '1.0.0'
        };

        console.log('[App] Attempting to register Metis service');
        await apiManager.registerAPI(metisService);
        console.log('[App] Metis Service registration successful');
        
        return true;
    } catch (error) {
        console.error('[App] Metis Service initialization failed:', error);
        return false;
    }
};
...
const initializeDependencies = () => {
  try {
    ...
    const domainManager = SpecializedDomainManager.getInstance();
